...
description: Execute a full implementation workflow from one or more Obsidian folder paths. Use when the user provides Obsidian folders or vault paths and wants Codex to use the Obsidian CLI, treat each file as one implementation unit, process folders in parallel when safe, create one branch per file, implement and review with GPT-5.5 Extra High, push each branch, and merge/push each completed file branch into main.
---
...
3. Use the installed Obsidian CLI to list/read files in each folder. Inspect `obsidian --help` first and use the CLI's native list/read commands when available. If the CLI cannot read file contents, use it to verify/resolve the vault or folder and then read files from disk as a fallback; report that fallback.
...
6. Treat each file as exactly one implementation unit. Do not split a file into smaller planned units unless the user explicitly asks.
...
8. For each file branch:
   ...
   - Inspect the existing code first and make minimal, scoped edits.
   ...
   - Commit only the files for that Obsidian file with a specific commit message.
...
- Never combine unrelated Obsidian files into one branch or commit just to move faster.
...
- If a file is ambiguous, infer conservatively from the codebase and note the assumption in the commit or final summary.
- If safe parallelism is not available, process folder lanes sequentially and say why in the final response.
- Stop and ask for user input if authentication, branch protection, missing remotes, unavailable Obsidian CLI, or irreconcilable merge conflicts prevent a safe push.

...
description: Execute an implementation workflow from one or more Obsidian folder paths without pushing. Use when the user provides Obsidian folders or vault paths and wants Codex to use the Obsidian CLI, treat each file as one implementation unit, process folders in parallel when safe, create one branch per file, implement and review with GPT-5.5 Extra High, and commit each file branch locally.
---
...
- Stop and ask for user input if authentication for an existing remote, unavailable Obsidian CLI, or irreconcilable branch conflicts prevent safe local commits.
